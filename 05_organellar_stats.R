...
for(l in POP_SPEC) {
  nam <- paste(l, sep = "")
  tmp1=paste("vcf_analyses/admixture/",l,".popgenome.indv", sep = "")
...
write.csv(Mito_TajimasD, file=file.path(here::here(), "results/organellar/mito/FILE_Mito_mapped_TajimasD.csv"))
Plastid <- neutrality.stats(Plastid)
Plastid_TajimasD <- Plastid@Tajima.D
write.csv(Plastid_TajimasD, file=file.path(here::here(), "results/organellar/plastid/FILE_Plastid_mapped_TajimasD.csv"))
...
write.csv(Mito_pi, file=file.path(here::here(), "results/organellar/mito/FILE_Mito_mapped_Pi.csv"))
Plastid <- F_ST.stats(Plastid)
Plastid_pi <- Plastid@nuc.diversity.within/Plastid@n.sites
write.csv(Plastid_pi, file=file.path(here::here(), "results/organellar/plastid/FILE_Plastid_mapped_Pi.csv"))
#done