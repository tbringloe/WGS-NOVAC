...
# source: https://github.com/speciationgenomics/scripts/blob/master/plotADMIXTURE.r
...
# Modified: Trevor Bringloe, March 2022
...
option_list = list(
  ...
  make_option(c("-r", "--RBpalette"), type="character", default=NULL, 
              help="RColorBrewer palette to use", metavar="character"),
...
minK=opt$minK
maxK=opt$maxK
tbl<-lapply(minK:maxK, function(x) read.table(file.path(here::here(), "vcf_analyses/admixture/", paste0(prefix,".",x,".Q"))))
...
pdf(file=file.path(here::here(), "results/admixture/", paste0(opt$outPrefix,"admixture.pdf")))
par(mfrow=c(maxK-1,1),mar=c(0,1,0,0),oma=c(2,1,9,1),mgp=c(0,0.2,0),xaxs="i",cex.lab=1.2,cex.axis=0.8)
RBpalette=opt$RBpalette
coul <- brewer.pal(maxK, RBpalette)
# Plot minK
bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=coul,xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6)
# Plot higher K values
if(maxK>minK)lapply(2:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=coul,xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces))
