...
TajimaD <- subset(TajimaD, TajimaD!="NaN")
TajimaD_scores <- aggregate(TajimaD2[, 4], list(TajimaD2$loc), mean)
colnames(TajimaD_scores)[1] <- "loc"
write.csv2(TajimaD_scores, file=file.path(here::here(), "vcf_analyses/popstats/TajimaD/FILE_TajimaD_mean.csv"))
#Plot data
...
pi <- read_delim(file.path(here::here(), "vcf_analyses/popstats/nucleotide_diversity/FILE.sites.pi"), delim = "\t", col_names = c("CHROM", "BIN_START", "BIN_END", "N_VARIANTS", "Pi", "loc"), skip = 1)
#correct for 0 values excluded by vcftools
#get mean site pi values
vcf_correction <- GENOME_SIZE/WINDOW_SIZE
pi_site_scores <- aggregate(pi[, 3], list(pi$loc), mean)
colnames(pi_site_scores)[1] <- "loc"
write.csv2(pi_site_scores, file=file.path(here::here(), "vcf_analyses/popstats/nucleotide_diversity/FILE_pi_site_mean.csv"))
...
pdf(file=file.path(here::here(), "results/nuclear_stats/FILE_windowed_pi.pdf"), width = 6, height = 3)
p <- ggplot(pi, aes(x=loc, y=Pi, fill=loc)) + geom_violin() + geom_point(data=pi_scores, shape=24, size=2)
p
dev.off()
#clean up environment
rm(list = ls())
#Import relatedness AJK data
relat <- read_table(file.path(here::here(), "vcf_analyses/popstats/relatedness/FILE.relatedness"))
individuals <- read.table("specimen_list")
...
pdf(file=file.path(here::here(), "results/nuclear_stats/relatedness/FILE_relatedness_AJK.pdf"), width = 10, height = 10)
...
relat2 <- read_table(file.path(here::here(), "vcf_analyses/popstats/relatedness/FILE.relatedness2"))
